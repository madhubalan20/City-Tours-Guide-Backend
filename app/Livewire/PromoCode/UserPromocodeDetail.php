...
class UserPromocodeDetail extends Component
{

    public $selectuser,$promocodelist, $coupon_code, $percentage, $maximum_discount_amount, $validate_date, $description,
    $minimum_order_amount, $flat_amount;

    public $edit_id, $edit_user_name, $edit_coupon_code, $edit_percentage, $edit_maximum_discount_amount, $edit_validate_date, 
    $edit_description, $edit_minimum_order_amount, $edit_flat_amount, $delete_id;
    ...
    public function render()
    {
        ...
        $this->promocodelist = PromoCode::where('coupon_type', '2')->orderBy('id', 'desc')->get();

        return view('livewire.promo-code.user-promocode-detail');
    }
    ...
    public function addCoupon(){
        ...
        try{
            ...
            if(count($this->user_name ) > 0){

                foreach($this->user_name as $user){

                    $updateCoupon = PromoCode::where('user_id', $user)->first();

                    if($updateCoupon){

                        $updateCoupon->user_id =$user;
                        $updateCoupon->tour_id =null;
                        $updateCoupon->coupon_type ='2';
                        $updateCoupon->coupon_code =$this->coupon_code;
                        $updateCoupon->minimum_order_amount =$this->minimum_order_amount;
                        $updateCoupon->percentage =$this->percentage;
                        $updateCoupon->maximum_discount_amount =$this->maximum_discount_amount;
                        $updateCoupon->flat_amount =$this->flat_amount;
                        $updateCoupon->validate_day =$this->validate_date;
                        $updateCoupon->description =$this->description;
                        ...
                        $newCoupon->user_id =$user;
                        $newCoupon->tour_id =null;
                        $newCoupon->coupon_type ='2';
            ...
            return redirect()->route('promocode.user.promocode')->with($notification);

        }catch(Exception $e){

            return redirect()->route('promocode.user.promocode')->with('error', $e->getMessage());

        }
    }

    public function editUserPromocode($id)
    {
